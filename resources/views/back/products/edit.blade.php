...
<div class="card card-primary card-outline">
  <div class="card-header">
    <h5 class="m-0">Edit Product: {{ $product->name }}</h5>
  </div>
  <div class="card-body">
    <form method="post" action="{{ route('admin.products.update', $product) }}">
      ...
      {{--  new field class_icon  --}}
        <div class="mb-3">
            ...
            <input type="text" name="icon_class" class="form-control" value="{{ old('icon_class', $product->icon_class) }}">
            @error('icon_class') <small class="text-danger">{{ $message }}</small> @enderror
        </div>
        {{--  new field save_text  --}}
        <div class="mb-3">
            <label class="form-label">Save Text</label>
            <input type="text" name="save_text" class="form-control" value="{{ old('save_text', $product->save_text) }}">
      ...
      <div class="mb-3">
        ...
        <input type="number" name="whmcs_product_id" class="form-control" value="{{ old('whmcs_product_id', $product->whmcs_product_id) }}" placeholder="WHMCS এর Product/Service ID (optional)">
