...
@section('breadcrumb','Add-ons / Edit')

@section('content')
@include('back.partials.flash')

<div class="card card-primary card-outline">
  <div class="card-header"><h5 class="m-0">Edit Add-on: {{ $addon->label }}</h5></div>
  <div class="card-body">
    <form method="post" action="{{ route('admin.add-ons.update', $addon) }}">
      @csrf @method('PUT')
      ...
      <div class="mb-3">
        ...
        <select name="category_id" class="form-select" required>
          ...
          @foreach($categories as $category)
            <option value="{{ $category->id }}" @selected(old('category_id', $addon->category_id) == $category->id)>{{ $category->name }}</option>
          @endforeach
        </select>
      ...
      <div class="mb-3">
        ...
        <input type="text" name="key" class="form-control" value="{{ old('key',$addon->key) }}" required>
        @error('key') <small class="text-danger">{{ $message }}</small> @enderror
      </div>

      <div class="mb-3">
        <label class="form-label">Label <span class="text-danger">*</span></label>
        <input type="text" name="label" class="form-control" value="{{ old('label',$addon->label) }}" required>
      ...
      <div class="row g-3">
        ...
        <div class="col-md-4">
          ...
          <input type="number" name="max_qty" class="form-control" value="{{ old('max_qty',$addon->max_qty) }}" min="1" max="65535" required>
