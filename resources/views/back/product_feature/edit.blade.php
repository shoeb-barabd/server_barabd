...
@section('page_title','Edit Product Feature')
@section('breadcrumb','Product Features / Edit')

@section('content')
@include('back.partials.flash')

<div class="card card-primary card-outline">
  <div class="card-header"><h5 class="m-0">Edit Feature: {{ $feature->label ?? $feature->key }}</h5></div>
  <div class="card-body">
    <form method="post" action="{{ route('admin.product-features.update', $feature) }}">
      @csrf @method('PUT')
      ...
      <div class="mb-3">
        ...
        <select name="product_id" class="form-select" required>
          ...
          @foreach($products as $p)
            <option value="{{ $p->id }}" @selected(old('product_id',$feature->product_id)==$p->id)>{{ $p->name }}</option>
          @endforeach
        </select>
      ...
      <div class="mb-3">
        ...
        <input type="text" name="label" class="form-control" value="{{ old('label',$feature->label) }}" required>
      ...
      <div class="row g-3">
        ...
        <div class="col-md-4">
          ...
          <input type="text" name="unit" class="form-control" value="{{ old('unit',$feature->unit) }}">
      ...
      <div class="mb-3 mt-3">
        ...
        <textarea name="options_json" class="form-control" rows="3">{{ old('options_json',$feature->options_json) }}</textarea>
