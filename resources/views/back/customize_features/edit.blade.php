@extends('layouts.app')
@section('title','Edit Customize Feature')
@section('page_title','Edit Customize Feature')
@section('breadcrumb','Customize Features / Edit')

@section('content')
@include('back.partials.flash')

<div class="card card-primary card-outline">
  <div class="card-header"><h5 class="m-0">Edit Customize Feature</h5></div>
  <div class="card-body">
    <form method="post" action="{{ route('admin.customize-features.update', $feature) }}">
      @csrf @method('PUT')
      ...
      <div class="mb-3">
        ...
        <select name="category_id" class="form-select" required>
          ...
          @foreach($categories as $c)
            <option value="{{ $c->id }}" @selected(old('category_id',$feature->category_id)==$c->id)>{{ $c->name }}</option>
          @endforeach
        </select>
